...
namespace Csorm2.Core.Cache
{
    public class CacheEntry
    {
        ...
        {
            if(!attribute.IsEntityType) throw new Exception("Attribute must be a relation in order to associate it with related keys");
            ...
            if (newRelatedObject is IEnumerable newRelatedObjects)
            {
                ...
                foreach (var (pk, obj) in pks)
                {
                    ...
                    if (_context.Cache.GetEntry(oneToMany.ToEntity, pk) == null)
                    {
                        ...
                        changes.Add(new ValueChange(oneToMany.ToEntity, oneToMany.ToKeyAttribute, obj, null, primaryKey));
                    }
                    else
                    {
                ...
                foreach (var key in relatedKeys)
                {
                    ...
                    changes.Add(new ValueChange(oneToMany.ToEntity, oneToMany.ToKeyAttribute, entry.Object, primaryKey, null));
                }
                return changes;
            }
        ...
        private IEnumerable<IValueChange> ExtractManyToManyChanges(ManyToMany manyToMany)
        {
            ...
            Trace.Assert(newValues != null, "related keys were loaded but relation value is null, should be empty list at worst");
            ...
            foreach (var entry in newValueEntries)
            {
